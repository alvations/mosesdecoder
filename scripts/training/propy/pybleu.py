...
def corpus_bleu(list_of_references, hypotheses, weights=(0.25, 0.25, 0.25, 0.25),
    ...
    bleu_output = ("BLEU = {}, {} (BP={}, ratio={}, hyp_len={}, ref_len={})".format(
                  round(score*100, 2), '/'.join(map(str, [round(float(p_i)*100, 1) for p_i in p_n_str])),
...
def chen_and_cherry(references, hypothesis, p_n, hyp_len, 
    ...
    if smoothing == 5:
        ...
        p_n_plus1 = p_n + [modified_precision(references, hypothesis, len(p_n)+1)]
        m[-1] = p_n[0] + 1
        for i, p_i in enumerate(p_n):
    ...
    # between pn and pn−1 will be the same as that between pn−1 and pn−2 (Gao and He, 2013).
    if smoothing == 6:
        for i, p_i in enumerate(p_n):
...
if __name__ == '__main__':
    ...
    io.open(hypothesis_file, 'r', encoding='utf8') as hypfin:
        ...
        corpus_bleu(list_of_references, hypotheses, 
                    ...
                    smoothing=smoothing_method, epsilon=epsilon, 
                    alpha=alpha, k=k) 
